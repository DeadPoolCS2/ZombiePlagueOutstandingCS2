...
#define is_valid_player(%1) (1 <= %1 <= 64)

new DG_V_MODEL[64] = "models/ZombieOutStanding/v_golden_deagle.mdl"
new DG_P_MODEL[64] = "models/ZombieOutStanding/p_golden_deagle.mdl"
...
public plugin_init()
{
	...
	/* CVARS */
	cvar_dmgmultiplier = register_cvar("zp_goldendg_dmg_multiplier", "1.3")
	cvar_custommodel = register_cvar("zp_goldendg_custom_model", "1")
	cvar_goldbullets = register_cvar("zp_goldendg_gold_bullets", "1")
	cvar_uclip = register_cvar("zp_goldendg_unlimited_clip", "0")
	
	// Register The Plugin
	register_plugin("[ZP] Extra: Golden Deagle", "1.1", "AlejandroSk")
	// Register Zombie Plague extra item
	g_itemid = zp_register_extra_item("Golden Deagle [\rNight Hawk\w]", 30, ZP_TEAM_HUMAN)
	// Death Msg
...
public zp_extra_item_selected(player, itemid)
{
	if ( itemid == g_itemid )
	{
		...
		set_hudmessage(random_num(0, 255), random_num(0, 255), random_num(0, 255), -1.0, 0.0, random_num(0, 2), random_float(0.7, 0.9), 12.0, random_float(0.37, 0.4), random_float(0.37, 0.4), 4);
		ShowSyncHudMsg(0, h_h, ".:: %s bought NightHawk ::.", h_Name);
		g_HasDg[player] = true;
	}
}
