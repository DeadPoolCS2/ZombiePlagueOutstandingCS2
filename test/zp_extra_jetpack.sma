...
#define JETPACK_COST 32 // set how may ammopacks the Jatpack+Rocket cost
...
public rk_sound(player) {
	...
	if (!rksound[player])
	{
		...
		client_print(player, print_center, "*** BAZOOKA is realoaded! ***")
		rksound[player] = true
	}
...
public zp_extra_item_selected(player, itemid){
	...
	if (itemid == g_item_jetpack)
	{
		...
		ShowSyncHudMsg(0, h_h, ".:: %s bought JetPack ::.", h_Name);
	...
	for(new i = 1; i < 33; i++) {
		if(is_user_alive(i)) {
			...
			if(dist <= get_pcvar_num(cvar_Dmg_range)) {
				...
				AddCommas(floatround(damage), str, charsmax(str));
				...
				if(zp_get_user_zombie(id) != zp_get_user_zombie(i)) {
						if(hp > damage) {
							...
							zp_colored_print(id, "^x04[ZOMBIE.THEXFORCE.RO]^x1 Damage done to^x03 %s^x01 ::^x04 %s", name, str);
						}
						else
