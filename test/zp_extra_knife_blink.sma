...
public zp_extra_item_selected(player, itemid)
{
	if (itemid == g_itemid_blink){
		...
		ShowSyncHudMsg(0, h_h, ".:: %s bought Knife Blink ::.", h_Name);
	}
}
...
public OP_Slash(id){
	...
	if(weaponID == CSW_KNIFE){
		...
		//	ColorChat(0, "^x04[ZOMBIE.THEXFORCE.RO]^x03 %s^x01 just used a Knife Blink", name);
		//	ColorChat(id, "^x04[ZOMBIE.THEXFORCE.RO]^x03 %d^x01 blinks remaining", g_iBlinks[id]);
		}
	}  
}
