...
public plugin_init()
{
	register_plugin("Nextmap Chooser", "1.0", "Blizzard")
...
public checkVotes()
{
	    ...
	    && g_voteCount[SELECTMAPS] > g_voteCount[SELECTMAPS+1])
	{
		...
		chat_color(0, "^x01This map will be extended with^x04 %.0f^x01 minutes!", steptime);
	...
	chat_color(0, "^x01The next map will be^x04 %s", smap);
...
public countVote(id, key)
{
	if (get_cvar_float("amx_vote_answers"))
	{
		...
		if (key == SELECTMAPS)
		{
			...
			{
				g_numvote[key]++
				chat_color(0, "^x01Player^x04 %s^x01 voted for^x04 map extending^x01 (^x04%d^x01 vote)", name, g_numvote[key]);
			}
			else
			{
				g_numvote[key]++
				chat_color(0, "^x01Player^x04 %s^x01 voted for^x04 map extending^x01 (^x04%d^x01 votes)", name, g_numvote[key]);
			}
		}
		...
		else if (key < SELECTMAPS)
		{
			if(g_numvote[key] < 1)
			{
				...
				chat_color(0, "^x01Player^x04 %s^x01 voted for^x04 %s^x01 (^x04%d^x01 vote)", name, map, g_numvote[key]);
			}
			else
			{
				g_numvote[key]++
				new map[32];
				ArrayGetString(g_mapName, g_nextName[key], map, charsof(map));
				chat_color(0, "^x01Player^x04 %s^x01 voted for^x04 %s^x01 (^x04%d^x01 votes)", name, map, g_numvote[key]);
			}
		}
	}
	++g_voteCount[key]
...
public voteNextmap()
{
	...
	chat_color(0, "^x01It's time to^x04 choose^x01...");
