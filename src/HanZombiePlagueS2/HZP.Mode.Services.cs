...
public partial class HZPServices
{
    ...
    /*
    public void InvisibilityTimer(IPlayer player, float configDist)
    {
        if (player == null || !player.IsValid)
            return;
        ...
        _globals.g_AssassinTimer[Id]?.Cancel();
        _globals.g_AssassinTimer[Id] = null;
        _globals.g_AssassinTimer[Id] = _core.Scheduler.RepeatBySeconds(0.2f, () =>
        {
            var assassin = _core.PlayerManager.GetPlayer(Id);
            if (assassin == null || !assassin.IsValid)
                return;
            ...
            _globals.IsAssassin.TryGetValue(Id, out bool currentIsAssassin);
            if (!currentIsAssassin) 
            {
                //_logger.LogInformation("[Debug]g_AssassinTimer 检查到 玩家已不是暗杀者，正在清理状态...");

                var currentPawn = assassin.PlayerPawn;
                if (currentPawn != null && currentPawn.IsValid)
                {
                    if (currentPawn.Render.A != 255)
                    {
                        currentPawn.Render.A = 255;
            ...
            var enemies = _core.PlayerManager.GetAlive();
            bool hasEnemyInRange = false;
            float distSqrThreshold = configDist * configDist;
            ...
            foreach (var enemy in enemies)
            {
                ...
                var enemyId = enemy.PlayerID;

                _globals.IsZombie.TryGetValue(enemyId, out bool IsZombie);
                if(IsZombie)
                    continue;
                ...
                float distanceSqr = _helpers.DistanceSquared(assassinOrigin.Value, enemyOrigin.Value);
                if (distanceSqr <= distSqrThreshold)
                {
                    hasEnemyInRange = true;
                    break;
                }
            ...
            if (!_globals.g_IsInvisible.ContainsKey(Id))
                _globals.g_IsInvisible[Id] = false;

            bool currentState = _globals.g_IsInvisible[Id];

            if (currentState != shouldBeInvisible)
            {
                if (shouldBeInvisible)
                    _helpers.SetInvisibility(assassin);
                else
        ...
        _core.Scheduler.StopOnMapChange(_globals.g_AssassinTimer[Id]);
